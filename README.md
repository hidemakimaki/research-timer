...
[ GitHub ]  ──── 自動デプロイ ────▶  [ Vercel ]
                  ...
                  メール+パスワード              sessions テーブル
                  ユーザー管理                  Row Level Security
                                              （自分のデータのみ読み書き可）
```

### データの流れ

1. コードを `git push` → Vercelが自動でビルド・デプロイ
2. ユーザーがVercelのURLにアクセス → Reactアプリが読み込まれる
3. Supabase Authでログイン → ユーザーIDが発行される
4. タイマーを終了すると → Supabaseのsessionsテーブルに保存
5. 別デバイスから同じアカウントでログイン → 同じ記録が取得できる
